...
- **实时主持** — 随时插话；你的发言会在下一位模型发言之前插入记录，因此所有参与者
  都能看到并作出回应。
- **每轮总结** — 每轮结束给出：共识点、仍存的分歧、新出现的洞见、待解决的问题，
  然后暂停等你决定。
- **跟随辩论语言** — 总结和结论会使用参与者实际使用的语言，**包括小标题**，不会出现
  中文正文配英文标题的情况。（一个刻意的例外：末尾那行 `**Convergence:**` 保持英文，
  因为程序要解析它来生成共识标签。）
- **可给评判模型下指令** — 两个模型选择框下各有一个折叠的输入框，可以用自然语言指定
  该角色的语气、长度、语言、关注重点等。和模型选择一样可在辩论中途修改，也会随预设
  一起保存。留空也没问题，默认行为已经是对的。
- **可自选评判模型** — 总结和结论由哪个模型撰写，可在界面中单独选择，与辩论参与者
  无关，并且**辩论过程中可随时更改**（在暂停点修改，从下一轮开始生效）。默认是
  **「自动」**，会挑选一个你实际已连接的供应商——因此即使你只配置了一个模型（例如
  只有 DeepSeek），也照样能得到总结和最终结论。每条总结和结论都会记录是哪个模型
  生成的，界面和导出中均可见。
...
- **账号面板** — 一眼看清每个供应商的连接状态，并给出登录所需的确切终端命令和
  重新检查按钮。
- **分层的模型成本** — 辩论发言可用便宜、快的模型，最终结论用更强的模型，因为综合
  判断的质量在这里最重要。只有 DeepSeek 的发言产生实际花费，而且只有前沿模型价格
  的约 1%。
...
4. 每轮结束：由你选定的总结模型（默认「自动」）撰写总结，然后暂停等你决定。
...
- **`prompts.local.json`** — 可选。用 `turn`、`summary`、`verdict` 三个键
  覆盖内置的提示词模板。该文件只在启动时读取一次，修改后需重启服务。小幅调整建议
  直接用界面上的指令输入框——即时生效，也不必整段替换提示词。每个模板通过 `.format()` 填入与默认版本相同的字段
